...
| [`crossjuris`](./crossjuris/) | `http.RoundTripper` that follows entire-core's cross-jurisdiction `421 Misdirected Request` to the `home_core_url` it names, replaying the buffered body once. Before following, the target host must appear in the responding core's `/.well-known/entire-federation` manifest (fetched lazily, cached per origin, negative results included). With `ClientID` set it also runs the RFC 8693 exchange a home core may still demand for a foreign-region login JWT; leave it empty for follow-only. Entire-specific by design: the redirect protocol has no RFC. |
...
- **Server-supplied redirect targets are gated.** `crossjuris` re-targets the user's login JWT only at a `home_core_url` that is HTTPS (loopback HTTP needs `AllowInsecureHTTP`) and whose host the responding core lists as a federation peer; a `token_exchange_url` must additionally be same-host with the response that carried it and end in `/oauth/token`. Anything else surfaces the server's original response unchanged.
- **HTTP redirects cannot carry a credential off-host or onto plaintext.** Every OAuth request this library makes refuses a redirect that leaves the host the caller targeted, and refuses any hop that leaves `https` once a chain has reached it. `net/http` strips sensitive *headers* on a redirect to a host that is not the original or a subdomain of it, but it replays the request *body* on 307/308 and permits a scheme change — and the body is where `subject_token`, `refresh_token`, an authorization code plus PKCE verifier, and `device_code` all live. So an open redirect in front of an otherwise legitimate token endpoint would otherwise hand them to a third host, or send them over the wire in clear. Same-host hops and `http`→`https` upgrades still follow. The device-authorization POST is exempt from the *host* restriction only, because its cross-host redirect is how regional routing works; it is still held to TLS, since its response carries a redeemable `device_code`.
