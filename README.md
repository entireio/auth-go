...
- **HTTP redirects cannot carry a credential off-host.** Every OAuth request this library makes refuses a redirect that leaves the host the caller targeted. `net/http` strips sensitive *headers* on a cross-host redirect but replays the request *body* on 307/308, and the body is where `subject_token`, `refresh_token`, an authorization code plus PKCE verifier, and `device_code` all live — so an open redirect in front of an otherwise legitimate token endpoint would otherwise hand them to a third host. Same-host hops still follow. The one exception is the device-authorization POST, whose body carries no secret and whose cross-host redirect is how regional routing works.
